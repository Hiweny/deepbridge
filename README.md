...
- **网站会话轮换（v1.6.1 调整）**：**压缩记忆不会开新对话**；只有距上次开新对话累计满「轮换阈值 R」（默认 200，按总对话轮数计）才自动 New chat，最大限度利用 DeepSeek 官网的上下文窗口。R 设为 0 即仅手动：微信 `/新对话`、控制台「新建对话」或对话页自己点。
...
| `/新对话` `/newchat` | 在 DeepSeek 官网手动开新对话（记忆保留） |
| `/重置` `/reset` | 清空本用户对话与记忆 |
| `/人设 描述` | 查看 / 修改角色人设 |
| `/多条开|关` | 多条消息模式开关 |
